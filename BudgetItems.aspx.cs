...
{
    public partial class BudgetItems : Page
    {
        ...
        {
            var dt = _Master.Sql.GetDataTable(_Master.SiteInfo.DatabaseName, "dbo", "spcfGetBudgetItems_020000", new NamedValue("UserId", 3));
            if (dt.Rows.Count > 0)
            {
                rprBudgetItems.DataSource = dt;
                rprBudgetItems.DataBind();
            }

            MainMultiview.SetActiveView(vwBudgetItems);
        }
        ...
        protected void rprBudgetItems_ItemDataBound(object sender, RepeaterItemEventArgs e)
        {
            ...
            {
                ((Image)e.Item.FindControl("imgIsAuto")).Visible = ((HiddenField)e.Item.FindControl("hidIsAuto")).Value == "1";
                ((Image)e.Item.FindControl("imgIsBill")).Visible = ((HiddenField)e.Item.FindControl("hidIsBill")).Value == "1";
                ((Image)e.Item.FindControl("imgIsLate")).Visible = ((HiddenField)e.Item.FindControl("hidIsLate")).Value == "1";
                try
                {
