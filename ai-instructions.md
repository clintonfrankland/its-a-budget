...
Use Semantic Versioning for every development project, including web apps and CLI tools.

### Current scheme

- Product, display, and package version: `MAJOR.MINOR.PATCH`.
- Assembly/file metadata may carry a separate build number when required by a deployment system; it must not replace the semantic project version.
...
- Use a literal `<Version>` or `VersionPrefix` in `MAJOR.MINOR.PATCH` form for shipped projects.
- If no version field exists yet, add a clear version property to the shipped project before calling the task done.

### When making changes

1. For bug fixes, increment `PATCH`.
2. For new features, increment `MINOR` when the feature is user-visible or operationally meaningful; reset `PATCH` to `0` when incrementing `MINOR`.
3. For breaking changes, increment `MAJOR` intentionally and document the reason.
4. Keep README/version references current when the versioning scheme or displayed version changes.
