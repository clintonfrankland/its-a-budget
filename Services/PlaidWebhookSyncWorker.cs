...
public sealed class PlaidWebhookSyncWorker(IServiceScopeFactory scopes, ILogger<PlaidWebhookSyncWorker> logger) : BackgroundService
{
    ...
    internal async Task DrainAsync(CancellationToken ct)
    {
        ...
        receipt.Status = "processing"; receipt.AttemptCount++; await db.SaveChangesAsync(ct);
        try
        {
            ...
            receipt.Status = "completed"; receipt.CompletedAtUtc = DateTime.UtcNow; receipt.ErrorCode = null;
        }
        catch (Exception ex)
        {
            receipt.Status = "queued"; receipt.ErrorCode = ex.GetType().Name;
        }
