...
Checkbook supports CSV backups and offline CSV, OFX/QFX, and QIF statement imports. **Export CSV** downloads every transaction the signed-in user can read. **Import** accepts a file up to 5 MiB and previews the first ten records, including cleared status and statement references, before importing every valid row into the displayed destination account. CSV retains configurable Date/Amount/Payee/Category mapping and starts uncleared; OFX/QFX posted entries are cleared; QIF retains its cleared marker. All formats preserve amount signs; omitted payees/categories use `Unknown` and `Uncategorized`. Unsupported statement structures or invalid rows prevent import. Re-importing a file creates duplicates: references are preview-only and there is no cross-upload deduplication. See [Checkbook import formats](docs/pages/checkbook.md#statement-import-formats) for supported structures and limits.
...
prohibit alternate writers, including manual pushes, PATs, deploy tokens, and
other workflows, from writing these tags. An alternate writer can still race a
preflight or alter a tag after verification; the workflow detects a mismatch it
observes, but cannot make the registry operation atomic.
...
5. Finish the guide, then use Manage → Budget Items for scheduled bills/income and detailed allowances, Profile for notification preferences, and Sharing for member roles.
...
| **Reports** | One read-only reporting destination with Overview, Spending, Cash Flow, and Net Worth sections. Spending expands selected-month recurring income, bills, and allowances by actual occurrence dates, reports each planned-versus-actual section separately, and excludes labelled transfers/card payments from spending variance. Variance is actual minus planned with favorable/unfavorable indicators and an overall net-cash-flow result. The former Insights route redirects to Spending. See [Reports](docs/pages/reports.md). |
...
- `MaxFileSizeBytes` defaults to `5242880` (5 MiB).
...
- **GHCR semantic tags are workflow-enforced immutable.** The publish workflow serializes its runs, refuses a semantic-version tag already owned by a different commit, and checks the pushed digest. A version bump is required before a different commit can publish; authentication or package-metadata errors fail closed. GHCR does not provide atomic tag compare-and-set, so repository administrators must not permit alternate writers for these tags.
...
Checkbook and Forecast now use per-budget in-flight Skip guards and the existing busy state on shared row-action controls. Repeated callbacks cannot overlap for the same budget; completion or failure releases the guard. Existing occurrence-aware service checks, permissions, editor routes, and compact controls are unchanged.
